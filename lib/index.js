...
const contextSchema = Joi.object()
    .keys({
        ...
        deviceType: Joi.string().allow('mobile', 'tablet', 'desktop').required(),
        query: Joi.object(),
        token: Joi.string().optional(),
        userId: Joi.string().optional(),
        sessionId: Joi.string().optional(),
        locale: Joi.string().optional().default('nb-NO'),
        cdnHost: Joi.string().optional().default('/'),
        extras: Joi.object()
            .keys({
...
const contentSchema = Joi.object()
    .keys({
        ...
        assetUris: Joi.array().items(Joi.string().uri().required()).optional(),
...
const resourceEntry = Joi.object()
    .keys({
        ...
        method: Joi.string().default('get').lowercase().optional(),
        params: Joi.array().optional().items(Joi.string()),
    })
    .unknown(false);
...
const resourceMountEntry = Joi.object()
    ...
    .unknown(false);
const maxAge = Joi.number().unit('seconds').min(0).max(60 * 60 * 60).required();

const maxDataAge = Joi.number().unit('seconds').min(0).max(60 * 60 * 60).less(Joi.ref('maxAge')).optional();
...
const metadataSchema = Joi.object()
    .keys({
        ...
        maxDataAge,
        maxAge,
        resources: Joi.array().items(resourceEntry).optional(),
    })
    .unknown(false);
...
const hostOptionsSchema = Joi.object()
    .keys({
        ...
        resources: Joi.array().default([]).items(resourceMountEntry),
        prepareContext: Joi.func().default((context /* , req*/) => Promise.resolve(Object.assign({}, context))),
